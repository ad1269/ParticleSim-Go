...
func applyForceOnParticles(particles []Particle, pointers [][]*Particle, dmin, davg *float64, navg *int, bins, start, end int, wg *sync.WaitGroup) {
	defer wg.Done()
	for i := start; i < end; i++ {
...
func moveParticles(particles []Particle, pointers [][]*Particle, bins, start, end int, wg *sync.WaitGroup, binLocks []*sync.Mutex) {
	defer wg.Done()
	for i := start; i < end; i++ {
		...
		if old_bin_j != new_bin_j || old_bin_i != new_bin_i {
			...
			binLocks[old_bin_i*bins+old_bin_j].Lock()
			binLocks[new_bin_i*bins+new_bin_j].Lock()
			...
			binLocks[old_bin_i*bins+old_bin_j].Unlock()
			binLocks[new_bin_i*bins+new_bin_j].Unlock()
...
func main() {
	...
	var PBLOCK_SIZE int = n / NUM_THREADS
	...
	wg := sync.WaitGroup{}
	binLocks := make([]*sync.Mutex, bins*bins)
	for i := range binLocks {
		lock := sync.Mutex{}
		binLocks[i] = &lock
	}
	...
	for step := 0; step < NSTEPS; step++ {
		...
		for i := 0; i < NUM_THREADS; i++ {
			if i != NUM_THREADS-1 {
				go applyForceOnParticles(particles, pointers, &dmin, &davg, &navg, bins, i*PBLOCK_SIZE, (i+1)*PBLOCK_SIZE, &wg)
			} else {
				go applyForceOnParticles(particles, pointers, &dmin, &davg, &navg, bins, i*PBLOCK_SIZE, n, &wg)
			}
		}
		wg.Wait()
		...
		for i := 0; i < NUM_THREADS; i++ {
			if i != NUM_THREADS-1 {
				go moveParticles(particles, pointers, bins, i*PBLOCK_SIZE, (i+1)*PBLOCK_SIZE, &wg, binLocks)
			} else {
				go moveParticles(particles, pointers, bins, i*PBLOCK_SIZE, n, &wg, binLocks)
			}
		}
		wg.Wait()
