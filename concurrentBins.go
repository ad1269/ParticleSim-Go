...
func applyForceOnParticlesInBin(pointers [][]*Particle, dmin, davg *float64, navg *int, bins, bin_i, bin_j int, wg *sync.WaitGroup) {
	defer wg.Done()
	for i := 0; i < len(pointers[bin_i*bins+bin_j]); i++ {

		var currentParticle *Particle = pointers[bin_i*bins+bin_j][i]
		Zero_acceleration(currentParticle)
		...
		for other_bin_i := Max(0, bin_i-1); other_bin_i <= Min(bins-1, bin_i+1); other_bin_i++ {
			for other_bin_j := Max(0, bin_j-1); other_bin_j <= Min(bins-1, bin_j+1); other_bin_j++ {
				for k := 0; k < len(pointers[other_bin_i*bins+other_bin_j]); k++ {
					...
					Apply_force(currentParticle, p, dmin, davg, navg)
				}
			}
		}
	}
...
// TODO: Write this
func moveParticlesInBin(particles []Particle, pointers [][]*Particle, bins, start, end int, wg *sync.WaitGroup, binLocks []*sync.Mutex) {
	defer wg.Done()
	for i := start; i < end; i++ {
...
func main() {
	...
	for step := 0; step < NSTEPS; step++ {
		...
		for i := 0; i < bins; i++ {
			for j := 0; j < bins; j++ {
				go applyForceOnParticlesInBin(pointers, &dmin, &davg, &navg, bins, i, j, &wg)
			}
		}
		wg.Wait()
